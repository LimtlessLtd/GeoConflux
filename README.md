...
- **Control is assessed from evidence that travels with the assessment, or it is not asserted.** Who
  holds a place is worked out from reports that are about who holds it — coded territorial change,
  and corroborated capture claims — never from reports of fighting, because event density says where
  fighting was reported rather than who holds the ground. Every assessment carries the identifiers of
  the records behind it and the age of the newest; past thirty days it reads as *last asserted* and
  says that silence is not evidence it still holds. Nothing is interpolated between assessed places,
  so there is no front line to mistake for one
  ([ADR 037](docs/adr/037-assessed-control.md)).
...
| `GET /api/control` | Who is assessed to hold each place, the records behind it, and how old they are |
...
| `Control:Window` | 90 days | How far back control evidence is read at all |
| `Control:StaleAfter` | 30 days | After this an assessment reads as *last asserted*, never as *held* |
| `Control:ContestWindow` | 14 days | How close two actors' claims must be before a place is called contested rather than changed hands |
| `Control:MinimumClaimSources` | 2 | Independent sources an uncoded claim needs before it asserts anything. Coded evidence is exempt. |
...
Thirteen limitations worth stating plainly:
...
- **Coverage is global in capability, not yet in fact.** The two datasets that would make it global
  ...
  order.
- **Assessed control is built and dormant.** The strongest evidence for it — a coder asserting
  territory changed hands — arrives only with an ACLED credential, and the claim path needs two
  independent sources reporting the same capture. A credential-free clone therefore assesses nothing,
  and the panel says that is a statement about what was collected rather than about the world. The
  model tier that would read administrative signals out of prose is specified and not built, and
  recorded as such: it never asserts on its own, so its absence changes no verdict.
